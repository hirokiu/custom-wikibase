...
const runtimeProvider=async()=>{const observation=await observationProvider.observe(),syncState=observation.syncState;return{contractVersion:'jwb-runtime-v1',distribution:{type:'japan-wikibase',version:'0.1.0-rc.1'},instance:{id:instanceId},endpoints:{mediawiki:'http://127.0.0.1:8280/wiki/',actionApi:'http://127.0.0.1:8280/api.php'},health:{state:syncState==='CURRENT'?'healthy':syncState==='ERROR'||syncState==='GAP_DETECTED'?'unhealthy':'degraded'},queryService:{enabled:true,backendType:config.backendType,logicalEndpoint:'http://127.0.0.1:8290/sparql',syncState,freshness:observation.freshness,servingGeneration:observation.servingGeneration},capabilities:{queryOptional:true,instanceStopPreservesData:true}};};
