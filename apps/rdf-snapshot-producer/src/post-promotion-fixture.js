...
await mutate({action:'delete',title:manifest.targetItem,token:session.token,reason:'U1-B bounded deletion qualification',watchlist:'nochange'},'JWB_POST_PROMOTION_DELETE_FAILED');
await mutate({action:'undelete',title:manifest.targetItem,token:session.token,reason:'U1-B bounded restoration qualification',watchlist:'nochange'},'JWB_POST_PROMOTION_UNDELETE_FAILED');
