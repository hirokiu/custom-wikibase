...
if (!['promote', 'rollback', 'restore', 'status'].includes(action) || process.argv.length !== 3)
...
try {
  const result = action === 'status' ? await coordinator.status()
    : await coordinator.promote({ mode: action === 'promote' ? 'PROMOTE' : action === 'restore' ? 'RESTORE' : 'ROLLBACK', crashAt });
