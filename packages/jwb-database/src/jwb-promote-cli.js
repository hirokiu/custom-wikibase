...
try{
  const sync=await pool.query("SELECT state,schema_state FROM rdf_generation_sync WHERE source_identity='jwb-standalone' AND generation_id='gen-b'");
  ...
  const repository=new PostgresServingPointerRepository({pool,queryServiceId:'jwb-standalone-query'}),current=await repository.get();
