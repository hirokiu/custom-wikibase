export class StandaloneRuntimeProvider{
  constructor({pool,backendType}){if(!pool||!['virtuoso','fuseki-tdb2','oxigraph'].includes(backendType))throw new Error('INVALID_RUNTIME_PROVIDER');this.pool=pool;this.backendType=backendType;}
  async observe(){const result=await this.pool.query("SELECT p.generation_id,s.state,s.schema_state,s.catchup_cursor_timestamp,s.catchup_cursor_rcid,src.ingestion_cursor_timestamp,src.ingestion_cursor_rcid,src.status source_status FROM rdf_serving_pointer p JOIN rdf_generation_sync s ON s.source_identity=p.source_identity AND s.generation_id=p.generation_id JOIN rdf_sync_source src ON src.source_identity=p.source_identity WHERE p.query_service_id='jwb-standalone-query'");if(result.rowCount!==1)throw new Error('STANDALONE_RUNTIME_STATE_MISSING');return runtimeObservation(result.rows[0]);}
}
